...
- 유튜브 채널은 각 칸 안에서 구독자가 많아 그 주제의 대표 채널로 꼽힐
  만한 유명한 채널부터 먼저 쓰십시오. 각 칸을 채우는 데 급급해 무명·소규모
  채널부터 나열하고 대형 채널을 빠뜨리면 안 됩니다 — 구독자 수는 나중에
  실제로 조회해 다시 순위를 매기지만, 애초에 후보에 없으면 그 조회 자체가
  불가능합니다. 그 주제라면 누구나 먼저 떠올릴 만한 채널을 빠짐없이
  넣었는지 스스로 확인한 뒤 나머지를 채우십시오.
