...
#
# 3시간이었는데, 깃허브 크론이 열 시간씩 밀리는 날에는 그 사이에 실행이
# 한 번도 없어 회차가 조용히 사라졌다. 08:00 회차가 08:00~11:00 에 아무도
# 깨우지 않으면 그날은 없던 일이 된다. 늦게 오는 것이 안 오는 것보다 낫다.
#
# 그래도 무제한은 아니다. 자정을 넘겨 어제치를 보내면 날짜가 어긋나고,
# 발송 이력이 하루 단위라 다음 회차와 뒤엉킨다. 그날 안으로만 따라잡는다.
MAX_LATE = timedelta(hours=14)
